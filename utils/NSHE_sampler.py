...
def gen_ns_instances(f_type_adj, adj, edge, t_info, ns_negrate=1):
    def _gen_type_adj():
        ...
        for line in lines:
            ...
            type_adj_[lr_type].add(ri_type)
            type_adj_[ri_type].add(lr_type)
        return type_adj_

    def _get_current_sample_types(prev_sampled_types, pos_ns_):
        current_sample_types = []
        for t in prev_sampled_types:
            for neighbor_t in type_adj[t]:
                if neighbor_t not in pos_ns_.keys():
    ...
    def _get_type_specific_node_samplers():
        ...
        for t in types:
            ...
            node_sampler_[t] = AliasSampling(probs=t_weights / np.sum(t_weights))
        return node_sampler_

    def _sample_pos_ns(k, target_t_):
        id_ = t_info[target_t_]['ind'].start + k  # id of the node
        pos_ns_ = {'target_type': target_t_, target_t_: id_}
        prev_sampled_types = set([target_t_])
        #
        current_sample_types = _get_current_sample_types(prev_sampled_types, pos_ns_)
        while len(current_sample_types) != 0:
            ...
            for _ in prev_sampled_types:
                ...
                for _id in prev_node_neighbors:
                    prev_neighbors.append(_id)
            prev_neighbors = np.array(prev_neighbors)
            if len(prev_neighbors) == 0:  # if there is no neighbor to select
                print('Node {} is a isolated point!!'.format(id_))
                return None
            for t in current_sample_types:
                # find neighbors of type t
                t_neighbors = prev_neighbors[[j in t_info[t]['ind'] for j in prev_neighbors]]
                # random select one as positive ns_instance
                if len(t_neighbors) == 0:  # if there is no neighbor to select
                    print('Node {} has no {} type point!!'.format(id_, t))
                    return None
                if len(t_neighbors) == 1:  # if there is only one selection
                    ...
                    raise ValueError('negative #neighbors')
                pos_ns_[t] = t_neighbors[r]
